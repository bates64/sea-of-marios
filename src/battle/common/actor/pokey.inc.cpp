...
// Actor Stats
constexpr s32 hp = 4;
constexpr s32 dmgSmashLarge = 2;
constexpr s32 dmgSmashMedium = 2;
constexpr s32 dmgSmashSmall = 2;
constexpr s32 dmgThrowPart = 2; // throw a body part at the player
constexpr s32 dmgLeap = 2; // jump onto the player, used when no more parts can be thrown
...
EvtScript EVS_Attack_GroundSmash = {
    ...
    Switch(LVar0)
        CaseEq(0)
            Set(LVar1, dmgSmashLarge)
        CaseEq(1)
            Set(LVar1, dmgSmashMedium)
        CaseDefault
            Set(LVar1, dmgSmashSmall)
    EndSwitch
    Call(SetGoalToTarget, ACTOR_SELF)
    Wait(2)
...
EvtScript EVS_TakeTurn = {
    ...
    Call(SetTargetActor, ACTOR_SELF, ACTOR_PLAYER)
    Call(EnemyTestTarget, ACTOR_SELF, LVar0, DAMAGE_TYPE_NO_CONTACT, 0, dmgThrowPart, BS_FLAGS1_TRIGGER_EVENTS)
    Switch(LVar0)
        CaseOrEq(HIT_RESULT_MISS)
        CaseOrEq(HIT_RESULT_LUCKY)
    ...
    Call(SetAnimation, ACTOR_SELF, PRT_PROJECTILE, ANIM_Pokey_Projectile)
    Call(JumpPartTo, ACTOR_SELF, PRT_PROJECTILE, LVar0, LVar1, LVar2, 0, TRUE)
    Call(EnemyDamageTarget, ACTOR_SELF, LVar0, DAMAGE_TYPE_NO_CONTACT, 0, DMG_STATUS_KEY(STATUS_FLAG_POISON, 3, 0), dmgThrowPart, BS_FLAGS1_TRIGGER_EVENTS)
    Call(GetActorVar, ACTOR_SELF, AVAR_Anim_Run, LVar1)
    Call(SetAnimation, ACTOR_SELF, PRT_MAIN, LVar1)
    Switch(LVar0)
        CaseOrEq(HIT_RESULT_HIT)
...
Formation SummonFormation = {
    ACTOR_BY_POS(battle::actor::Pokey, SummonPos, 100),
};
...
EvtScript EVS_Attack_SinglePartLeap = {
    ...
    Call(SetAnimation, ACTOR_SELF, PRT_MAIN, ANIM_Pokey_Run1)
    Call(EnemyTestTarget, ACTOR_SELF, LVar0, 0, 0, dmgLeap, BS_FLAGS1_INCLUDE_POWER_UPS)
    Switch(LVar0)
        CaseOrEq(HIT_RESULT_MISS)
        CaseOrEq(HIT_RESULT_LUCKY)
            Set(LVarA, LVar0)
    ...
    Wait(2)
    Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, 0, DMG_STATUS_KEY(STATUS_FLAG_POISON, 3, 0), dmgLeap, BS_FLAGS1_TRIGGER_EVENTS)
    Call(SetAnimation, ACTOR_SELF, PRT_MAIN, ANIM_Pokey_Idle1)
    Switch(LVar0)
        CaseOrEq(HIT_RESULT_HIT)
        CaseOrEq(HIT_RESULT_NO_DAMAGE)
